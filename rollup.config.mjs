...
const packageJson = JSON.parse(
  fs
    .readFileSync(
      path.resolve(__dirname, "node_modules/@angular/upgrade/package.json"),
    )
    .toString(),
);
...
const packages = ["2022"]
  .map((ecma) => [
    {
      ecma,
      angularPackage: "@angular/upgrade",
      filename: "upgrade",
    },
    {
      ecma,
      angularPackage: "@angular/upgrade/static",
      filename: "static",
    },
  ])
  .flat();
...
function createConfig({ ecma, prod, format, angularPackage, filename }) {
  ...
  return {
    input: path.join(
      __dirname,
      `node_modules/@angular/upgrade/fesm${ecma}/${filename}.mjs`,
    ),
    output: {
